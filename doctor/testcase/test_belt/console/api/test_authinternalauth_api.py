...
route: prefix=console-interna... """
    ...
    def test_api_ConsoleInternalAuthService_GetWhitelistAccounts(self, config_obj, AuthinternalauthApi):
        """  运营后台查询白名单账户列表
route: prefix=console-internal actio... """
        page_request_offset = 0
        page_request_limit = 10
        page_request_total = None
        resp = AuthinternalauthApi.ConsoleInternalAuthService_GetWhitelistAccountsGetApi(page_request_offset=page_request_offset, page_request_limit=page_request_limit, page_request_total=page_request_total)
        assert resp.status_code == 200

    @pytest.mark.skip("post接口在场景用例中实现")
    def test_api_ConsoleInternalAuthService_CreateWhitelistAccount(self, config_obj, AuthinternalauthApi):
        """  运营后台创建白名单账户
route: prefix=console-internal action=... """
        resp = AuthinternalauthApi.ConsoleInternalAuthService_CreateWhitelistAccountPostApi()
        assert resp.status_code == 200
    @pytest.mark.skip("post接口在场景用例中实现")
    def test_api_ConsoleInternalAuthService_DeleteWhitelistAccount(self, config_obj, AuthinternalauthApi):
        """  运营后台移除白名单账户
route: prefix=console-internal action=... """
        user_id = None
        resp = AuthinternalauthApi.ConsoleInternalAuthService_DeleteWhitelistAccountPostApi(user_id=user_id)
        assert resp.status_code == 200