...
class Accuracy:
    def __init__(self,config):
        ...
        #文件头
        self.excel_header=["测试数据","检出code","实际code","code TP（完全匹配)","code FP(存在错误)","code FN(没检出)","备注"]
        #汇总信息头
    ...
    def calculte(self):
        ...
        for image_name,detect_result in self.detect_result.items():
            ...
            if image_name in self.ground_truth_data.keys():
                ...
                FP=0
                FN=0
                excel_each_row_data.append(detect_result["code"])
                excel_each_row_data.append(self.ground_truth_data[image_name]["code"])
                if detect_result["code"]==self.ground_truth_data[image_name]["code"]:
                    TP=1
                    name_SUM_TP=name_SUM_TP+TP
                elif detect_result["code"]=="":
                    FN=1
        ...
        #owner
        excel_sum_row_data=[]          
        excel_sum_row_data.append("code")
    ...
    def detect_run(self):
        ...
        for image_path_t in video_paths_t:
            ...
            resp = self.OcrApi.OCRService_OCRBusinessLicensePostApi(image=base64_image, encrypt_info=encrypt_info)
            ...
            if "business_license" not in resp.resp_json:
                value={
                "code": "",
                }
                print(f"检出无字段！！！")
            else:
                if "code" not in resp.resp_json["business_license"].keys():
                    value["code"]=""
                    print(f"检出无code字段！！！")
                else:
                    value["code"]=resp.resp_json["business_license"]["code"]     
...
if __name__=="__main__":
    #输入1--标注文件
    ground_truth_file="/Users/weishuting/code/belt-beta-new/doctor/resource/images/ids_image/ocr/business_license/ground_truth.json"
    #输入2--素材
    test_image_file="/Users/weishuting/code/belt-beta-new/doctor/resource/images/ids_image/ocr/business_license/test_image.json"
    #输入3--host
