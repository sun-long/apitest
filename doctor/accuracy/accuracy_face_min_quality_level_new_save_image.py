...
class Accuracy:
    def __init__(self,config):
        ...
        #测试结果图片存储文件夹名称
        self.accuracy_dir="quality_level"
    ...
    def init_excle(self):
        ...
        self.quality_level_dir=os.path.join(self.result_dir,str(nowTime)+"_"+self.accuracy_dir)   
        if not os.path.exists(self.quality_level_dir):
            os.makedirs(self.quality_level_dir)
    ...
    def calculte(self):
        ...
        #写入详细结果
        for image_path_t,detect_result in self.detect_result.items():
            excel_each_row_data=[]
            dirname,full_file_name=os.path.split(image_path_t)
            if full_file_name in self.ground_truth_data.keys():
                image_SUM=image_SUM+1
                excel_each_row_data.append(full_file_name)
                #检出quality_level  
                excel_each_row_data.append(self.ground_truth_data[full_file_name]["face_quality_level"])
                excel_each_row_data.append(detect_result["LAST"])
                save_image_according_level(self.quality_level_dir,image_path_t,detect_result["LAST"])
                ...
                print("该图片：%s"%full_file_name+"无对应的标注数据！！！")        
    ...
    def detect_run(self):
        ...
        for image_path_t in video_paths_t:
            ...
            dirname,full_file_name=os.path.split(image_path_t)
            self.detect_result[image_path_t]=value
...
if __name__=="__main__":
    #输入1--标注文件
    ground_truth_file="/Users/weishuting/scp/清晰度/ground_truth.json"
    #输入2--素材
    test_image_file=f"/Users/weishuting/scp/清晰度/test_image.json"
    #输入3--hostg
