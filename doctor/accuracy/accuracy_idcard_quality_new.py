...
class Accuracy:
    def __init__(self,config):
        ...
        #文件头
        self.excel_header=["测试数据","实际quality_level","实际quality_item","检出quality_level","检出quality_items","“来源”","原始分数"]
        #汇总信息头
    ...
    def calculte(self):
        ...
        for image_name,detect_result in self.detect_result.items():
            ...
            if image_name in self.ground_truth_data.keys():
                ...
                quality_items_string=json.dumps(detect_result["quality_items"])
    ...
    def detect_run(self):
        ...
        for image_path_t in video_paths_t:
            ...
            if "quality_items" not in resp.resp_json:
                value["quality_items"]=""
            else:
                value["quality_items"]=resp.json_get("quality_items")
