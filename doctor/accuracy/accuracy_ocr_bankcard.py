...
class Accuracy:
    def __init__(self,config):
        ...
        #测试结果文件名称
        self.result_name="ocr_bankcard.xlsx"
        #最终测试结果路径
        self.excel_save_path=None
        #测试结果excel的sheet_name
        self.SHEET_NAME=f"测试结果"
        #文件头
        self.excel_header=["测试数据","检出card_number","实际card_number","TP（完全匹配）","FP(存在错误)","FN(没检出)","备注"]
        #汇总信息头
    ...
    def calculte(self):
        ...
        for image_name,detect_result in self.detect_result.items():
            ...
            if image_name in self.ground_truth_data.keys():
                ...
                FP=0
                FN=0
                excel_each_row_data.append(detect_result["card_number"])
                excel_each_row_data.append(self.ground_truth_data[image_name]["card_number"])
                if detect_result["card_number"]==self.ground_truth_data[image_name]["card_number"]:
                    TP=1
                    name_SUM_TP=name_SUM_TP+TP
                elif detect_result["card_number"]=="":
                    FN=1
                    name_SUM_FN=name_SUM_FN+FN                 
                elif detect_result["card_number"]!=self.ground_truth_data[image_name]["card_number"]:
                    FP=1
        ...
        #name
        excel_sum_row_data=[]          
        excel_sum_row_data.append("card_number")
    ...
    def detect_run(self):
        ...
        for image_path_t in video_paths_t:
            ...
            resp = self.OcrApi.OCRService_OCRBankcardPostApi(image=base64_image, encrypt_info=encrypt_info)
            ...
            if "bankcard" not in resp.resp_json:
                value={
                "card_number": ""
                }
                print(f"检出无idcard字段！！！")
            else:
                if "card_number" not in resp.resp_json["bankcard"].keys():
                    value["card_number"]=""
                    print(f"检出无name字段！！！")
                else:
                    value["card_number"]=resp.resp_json["bankcard"]["card_number"]     
...
if __name__=="__main__":
    #输入1--标注文件
    ground_truth_file="/Users/weishuting/scp/viper/data/银行卡/2023-1-3-送标文件-done/ground_truth.json"
    #输入2--素材
    test_image_file="/Users/weishuting/scp/viper/data/银行卡/2023-1-3-送标文件-done/test_image.json"
    #输入3--host
