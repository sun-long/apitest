...
class GenTemplateSwaggerTool(object):
            ...
            for method_name, methodInfo in srvInfo.items():
                ...
                if methodInfo['method'] in ['post', 'put', 'patch']:
                    ...
                    if intef.files:
                        if 'header' in intef.body:
                            intef.update_body('header')  # 移除header
                        for key, value in intef.files.items():
                            if key not in param_name_list:
                                param_list_str += ", %s=None" % key
                            body_code_str += "        intef.files[\"%s\"] = %s\n" % (key, key)
    ...
    def genTestCaseTemplate(self, product_name):
        ...
        swagger_dir = os.path.join(config.swagger_path, self.swagger_dir)
        case_map_path = os.path.join(swagger_dir, "case_map.csv")
        case_map = {}
        if os.path.exists(case_map_path):
            case_map_list = utils.read_csv(case_map_path)
            for line in case_map_list:
                if not line:
                    continue
                if line[0].strip().startswith("#"):
                    continue
                print(line)
                case_map[line[1].strip()] = line[0].strip()
        ...
        for srv_name, srvInfo in self.collections.items.items():
            ...
            for method_name, methodInfo in srvInfo.items():
                ...
                if methodInfo['method'] in ['get', 'delete']:
                    if 'parameters' in methodInfo and methodInfo['parameters']:
                        for param_info in methodInfo['parameters']:
                            ...
                            invalid_param_list_str += " " * 8 + "('%s', 'invalid%s', 400, ''),\n" % (self.filter_str(param_info['name']),
                                                                                            self.filter_str(param_info['name']))
                            invalid_param_list_str += " " * 8 + "('%s', '', 400, ''),\n" % self.filter_str(param_info['name'])
                            invalid_param_list_str += " " * 8 + "('%s', None, 400, ''),\n" % self.filter_str(param_info['name'])
                ...
                if methodInfo['method'] in ['post', 'put', 'patch']:
                    ...
                    if intef.files:
                        for key, value in intef.files.items():
                            ...
                            invalid_param_replace_str += " " * 8 + "if invalidParam[0] == '%s':\n" % key
                            invalid_param_replace_str += " " * 8 + "    %s = invalidParam[1]\n" % key
                        invalid_file_code_str = "        intef.files[invalidParam[0]] = invalidParam[1]\n"
                ...
                param_func_text = param_func_text.replace('$$%%INVALID_FILE_CODE%%$$', invalid_file_code_str)
            ...
            if srv_name in case_map:
                case_dir = "cases/test_%s/%s" % (self.swagger_dir, case_map[srv_name])
            else:
                case_dir = "cases/test_%s/undefined" % self.swagger_dir

            api_save_dir = os.path.join(config.temp_template_path, self.swagger_dir, case_dir, "api")
            param_save_dir = os.path.join(config.temp_template_path, self.swagger_dir, case_dir, "param")
            scenario_save_dir = os.path.join(config.temp_template_path, self.swagger_dir, case_dir, "scenario")
...
if __name__ == '__main__':
    gts = GenTemplateSwaggerTool('nova')
