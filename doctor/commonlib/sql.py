...
class SQL(object):
    ...
    def __valuefix(self, value):
        ...
        @note:
        """
        value = re.sub(r"\'", "''", value) if type(value) == type("string") \
                or type(value) == type(u"unicode") else value
        return value
        ...
        for statement in queue:
            ...
            if(self.__check("data") and statement == "data:save"):
                ...
                for data in self.__protected["__data"]:
                    sets = sets + "%s = '%s', " % \
                        (data, self.__valuefix(self.__protected["__data"][data]))
    ...
    def grasp(self, sql):
        ...
        @note:
        """
        select_regx = re.compile("SELECT (COUNT\()?(?P<field>[\w\*\s\.,]+)\)? FROM"
                                 " (?P<table_name>.*?)(LIMIT|ORDER|GROUP|"
                                 "HAVING|WHERE|LEFT|RIGHT|INNER|$)", re.I)
        where_complex_regx = re.compile("WHERE (?P<condition>.*?)(LIMIT|"
                                        "ORDER|GROUP|HAVING|LEFT|RIGHT|INNER)", re.I)
        ...
        order_regx = re.compile("ORDER BY (?P<order_by>[\w\.\,\s]+\s+(ASC|DESC|\(\)|\s))\s*"
                                "(LIMIT|GROUP|HAVING|WHERE|LEFT|RIGHT|INNER|$)", re.I)
        insert_regx = re.compile("INSERT INTO (?P<table_name>\w+) \(((\w+,?\s?)+)\) VALUES "
                                 "\((([\"']?\w+[\"']?,?\s?)+)\)", re.I)
        update_complex_regx = re.compile("UPDATE (?P<table_name>\w+) SET (.*?)(LIMIT|ORDER|GROUP"
                                         "|HAVING|WHERE|LEFT|RIGHT|INNER)", re.I)
        update_regx = re.compile("UPDATE (?P<table_name>\w+) SET (.*)", re.I)
        table_regx = re.compile("FROM (?P<table_name>.*?)(LIMIT|ORDER|GROUP|HAVING|WHERE|LEFT"
                                "|RIGHT|INNER|$)", re.I)
        join_regx = re.compile("(?P<join_condition>(?P<join_dir>LEFT|RIGHT"
                               ")?\s*(?P<join_type>INNER|OUTER)? JOIN (?P<table_name>\w+)"
                               " (AS \w+\s+)?ON (.*?))(LIMIT|ORDER|GROUP|HAVING|WHERE)", re.I)
    ...
    def count(self, cheat=False):
        ...
        if(not group_having_regx.search(sql)):
            return self.db.fetchone(sql)[0] if not cheat else sql
        else:
    ...
    def save(self, cheat=False):
        ...
        sql = self.__sqlbuild(sql, ["data:save", "where", "order", "limit"])
        sql = self.__sqlfix(sql)
        self.__close()
        return self.db.execute(sql) if not cheat else sql
    ...
    def add(self, cheat=False):
        ...
        sql = self.__sqlfix(sql)
        self.__close()
        return self.db.insert(sql) if not cheat else sql
    ...
    def where_in(field, _list):
        ...
        for x in _list:
            temp += "'%s'," % x
        return "%s in (%s)" % (field, temp[:-1])

    def show_all_column_name(self):
        """
        :return:
        """
    ...
    def _insert_list(self, data_list):
        """
        :param data_list:
        :return:
        """
        sql_framework = "insert into %s(%s) values %s;"
        sets = ""
        ...
        for data in data_list:
            ...
            values = values + "(%s), " % line
        values = values.strip().rstrip(",")
        sql = sql_framework % (self.table_name, sets, values)
        self.query(sql)

    def insert_list(self, data_list, max_number=2000):
        """
        :param data_list:
        :param max_number:
        :return:
        ...
        for data in data_list:
            if num >= max_number:
                self._insert_list(result_list)
                # print "insert data %s line." % num
                num, result_list = 0, []
            else:
        ...
        if num > 0:
            self._insert_list(result_list)
            # print "insert data %s line." % num
        print("insert data %s line." % count)
