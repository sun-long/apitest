...
from defines.belt.api.galaxyoplog_service_swagger import GalaxyoplogSwaggerApi
...
class GalaxyoplogSwaggerBusiness(GalaxyoplogSwaggerApi):
    """ 业务类代码写在这里"""
    def __init__(self, host, token=None, config_obj=None, ext_info=None, user_info=None):
        super(GalaxyoplogSwaggerBusiness, self).__init__(host, token, config_obj, ext_info)
    ...
    def init_interface(self, inte_obj):
        ...
        if 'v1' in inte_obj.path:
            ...
            inte_obj.set_headers("accessToken", token)
        else:
