...
from defines.nova.api.service_service_swagger import ServiceSwaggerApi
...
class ServiceSwaggerBusiness(ServiceSwaggerApi):
    """ 业务类代码写在这里"""
    def __init__(self, host, token=None, config_obj=None, ext_info=None):
        super(ServiceSwaggerBusiness, self).__init__(host, token, config_obj, ext_info)
    ...
    def init_interface(self, inte_obj):
        ...
        if inte_obj.path == '/v1/ListInternalModel':
            inte_obj.set_headers('X-Sensenova-Action', 'ListInternalModel')
            inte_obj.set_headers('X-Sensenova-Version', 'v1')
        elif inte_obj.path == '/v1/GetInternalModel':
            inte_obj.set_headers('X-Sensenova-Action', 'GetInternalModel')
            inte_obj.set_headers('X-Sensenova-Version', 'v1')